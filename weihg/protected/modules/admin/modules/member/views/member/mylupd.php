...
$(function(){
	$('#memadd').click(function(){
		...
		if(name==''){
			...
			$.ajax({
				type: "POST",
				url: "/admin/member/member/membermylupdsave",
				data: {'name':name,'mobile':mobile,'sex':sex,'qq':qq,'cid':cid,'wed_t':wed_t,'address':address,'allot_desc':allot_desc,"mid":<?php echo isset($memupd)?$memupd->id:''?>,'from':from},
    ...
    <table class="same_t table">
		...
		<tr>
		  ...
		  <tr>
		    ...
		    <td width="37%" height="40" style="text-align:left">
		    	<input type="text" id="from" name="from" value="<?php echo isset($memupd)?str_replace('(后台录入)','',$memupd->entrance):''?>" />
		    	<!--表单验证失败显示错误信息-->

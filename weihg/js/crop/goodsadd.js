   ...
   //$("#init").click(function(e){
	...
	$(function(e){
        ...
        $field.uploadify({
            ...
            ,'uploader'  : "/admin/etp/handle/etpgoodsupsave"
            ,'auto'      : false    
            ,'multi'     : false   
            ,'method'    : 'post'
            ,'fileObjName' : 'upload'
            ...
            ,'onUploadSuccess' : function(file, data, response){  //上传成功后的触发事件
                ...
                if( rst.status == 0 ){
                    ...
                    var $image = $("<img src='"+imageData.path+"' id='image-uploaded' data-width='"+imageData.width+"' data-height='"+imageData.height+"' data-name='"+imageData.name+"'  data-path='"+imageData.path+"'/>");
   ...
   //注释以上代码,不用点击接钮上传,让其选择图片后自动上传
    ...
    $("#cutInit").click(function(e){
        ...
        $("#cut").show().click(function(e){
			...
			data['path'] = $uploaded.data('path');
            $.ajax({
                url:'/admin/etp/handle/etpcut',
                type:'POST',
                data:data,
